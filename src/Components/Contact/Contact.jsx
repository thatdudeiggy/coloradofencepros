...
const contactDetails = [
  {
    icon: <FiPhone className="text-green-400 text-3xl" />,
    title: "Phone",
    value: "(+00) 1234 5678 90",
  },
  {
    icon: <FiMail className="text-green-400 text-3xl" />,
    title: "Email",
...
const contactMeta = {
  ...
  mailto: "mailto:mymail@gmail.com",
};

const Contact = () => {
  const handleSend = () => {
    window.location.href = contactMeta.mailto;
  };
  ...
  return (
    <section className="relative py-36 px-6 md:px-20 transition-colors">
      ...
      <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        >
          {contactDetails.map(({ icon, title, value }, i) => (
            <div key={i} className="flex items-start gap-4">
              {icon}
              <div>
                <h4 className="dark:text-green-500 text-lg font-semibold mb-1">
                  {title}
                </h4>
